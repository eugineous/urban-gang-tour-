...
const EMPTY = { slug: "", name: "", bio: "", photo: "", genre: "", instagram: "", associatedStops: "" };

export default function PerformersAdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/performers").then(r => r.json()).then(data => { setItems(Array.isArray(data) ? data : []); setLoading(false); });
  }, []);
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/performers", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  async function del(item) {
    ...
    await fetch("/api/admin/performers", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: item.slug, password: pw }) });
    setItems(prev => prev.filter(x => x.slug !== item.slug));
  }

  return (
    <div style={S.page}>
      <h1 style={S.h1}>Performers</h1>
      <p style={S.sub}>Manage artists and performers per stop.</p>
      <AdminPwPrompt pw={pw} setPw={setPw} />

      <div style={S.card}>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 20px" }}>{editing ? "Edit Performer" : "Add Performer"}</h2>
        <div style={S.row}>
        ...
        <div style={S.row}>
          <div><label style={S.label}>Genre</label><input style={S.input} value={form.genre} onChange={e => set("genre", e.target.value)} placeholder="Afrobeats, House, etc." /></div>
        ...
        <div style={{ marginBottom: 16 }}><label style={S.label}>Bio</label><textarea style={{ ...S.input, height: 80, resize: "vertical" }} value={form.bio} onChange={e => set("bio", e.target.value)} /></div>
        <div style={{ marginBottom: 20 }}><label style={S.label}>Associated Stops (comma-separated slugs)</label><input style={S.input} value={form.associatedStops} onChange={e => set("associatedStops", e.target.value)} placeholder="nairobi-2025, mombasa-2025" /></div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button style={S.saveBtn} onClick={save}>{editing ? "Update" : "Add Performer"}</button>
      ...
      <div>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 16px" }}>All Performers ({items.length})</h2>
        {loading ? <p style={{ color: "rgba(255,255,255,0.4)" }}>Loading…</p> : items.length === 0 ? <p style={{ color: "rgba(255,255,255,0.4)" }}>No performers yet.</p> : items.map(item => (
          <div key={item.slug} style={{ ...S.card, display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 2 }}>{item.genre} {item.instagram && `· ${item.instagram}`}</div>
            </div>
