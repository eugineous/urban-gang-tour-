...
const EMPTY = { slug: "", name: "", role: "", note: "", photo: "", instagram: "", profileSlug: "" };
...
export default function CrewAdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/crew").then(r => r.json()).then(data => { setItems(Array.isArray(data) ? data : []); setLoading(false); });
  }, []);
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/crew", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, password: pw }) });
  ...
  async function del(item) {
    ...
    await fetch("/api/admin/crew", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: item.slug, password: pw }) });
    setItems(prev => prev.filter(x => x.slug !== item.slug));
  }

  return (
    <div style={S.page}>
      <h1 style={S.h1}>Crew</h1>
      <p style={S.sub}>Manage crew members. <a href="/crew" target="_blank" style={{ color: "#cc0077" }}>View /crew →</a></p>
      <AdminPwPrompt pw={pw} setPw={setPw} />

      <div style={S.card}>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 20px" }}>{editing ? "Edit Crew Member" : "Add Crew Member"}</h2>
        <div style={S.row}>
          <div><label style={S.label}>Slug *</label><input style={S.input} value={form.slug} onChange={e => set("slug", e.target.value)} placeholder="e.g. dj-xavi" disabled={editing} /></div>
          <div><label style={S.label}>Name *</label><input style={S.input} value={form.name} onChange={e => set("name", e.target.value)} placeholder="DJ Xavi" /></div>
        </div>
        <div style={S.row}>
          <div><label style={S.label}>Role *</label><input style={S.input} value={form.role} onChange={e => set("role", e.target.value)} placeholder="DJ / MC" /></div>
          <div><label style={S.label}>Profile Slug</label><input style={S.input} value={form.profileSlug} onChange={e => set("profileSlug", e.target.value)} placeholder="Links to /people/[slug]" /></div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}><label style={S.label}>Instagram</label><input style={S.input} value={form.instagram} onChange={e => set("instagram", e.target.value)} placeholder="@handle" /></div>
        <div style={{ marginBottom: 20 }}><label style={S.label}>Note</label><input style={S.input} value={form.note} onChange={e => set("note", e.target.value)} placeholder="Short bio or note" /></div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button style={S.saveBtn} onClick={save}>{editing ? "Update" : "Add Crew"}</button>
      ...
      <div>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 16px" }}>All Crew ({items.length})</h2>
        {loading ? <p style={{ color: "rgba(255,255,255,0.4)" }}>Loading…</p> : items.length === 0 ? <p style={{ color: "rgba(255,255,255,0.4)" }}>No crew yet.</p> : items.map(item => (
          <div key={item.slug} style={{ ...S.card, display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", marginTop: 2 }}>{item.role} {item.instagram && `· ${item.instagram}`}</div>
            </div>
