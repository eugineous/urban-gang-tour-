...
export async function GET() {
  try {
    const keys = await redis.smembers("ugt:partners:index");
...
export async function POST(req) {
  ...
  const key = `ugt:partners:${slug}`;
  const record = { ...body, slug, updatedAt: new Date().toISOString() };
  delete record.password;

  try {
    await redis.set(key, JSON.stringify(record));
    await redis.sadd("ugt:partners:index", key);
...
export async function DELETE(req) {
  ...
  try {
    await redis.del(key);
    await redis.srem("ugt:partners:index", key);
