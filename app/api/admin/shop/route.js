...
export async function GET() {
  try {
    const keys = await redis.smembers("ugt:shop:index");
...
export async function POST(req) {
  ...
  const { password, id, name, price } = body;
...
export async function DELETE(req) {
  ...
  try {
    await redis.del(key);
    await redis.srem("ugt:shop:index", key);
