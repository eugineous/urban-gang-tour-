...
export async function GET() {
  try {
    const keys = await redis.smembers("ugt:people:index");
...
export async function POST(req) {
  ...
  const { password, slug, name, role } = body;
...
export async function DELETE(req) {
  ...
  try {
    await redis.del(key);
    await redis.srem("ugt:people:index", key);
