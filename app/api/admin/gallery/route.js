...
export async function GET() {
  try {
    const keys = await redis.smembers("ugt:gallery:index");
...
export async function POST(req) {
  ...
  const { password, slug, title } = body;
...
export async function DELETE(req) {
  ...
  try {
    await redis.del(key);
    await redis.srem("ugt:gallery:index", key);
